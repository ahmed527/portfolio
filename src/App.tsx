	...
	return (
		...
		>
			...
			>
				...
				{isMenuOpen && (
					<div
						className={`md:hidden ${darkMode ? 'bg-slate-900' : 'bg-white'} border-b ${
			...
			<section
				id='home'
				className='pt-32 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-12'
			>
				<div className='flex-1 space-y-8 animate-fade-in-up'>
					...
					>
						...
						<span className='text-emerald-400 font-semibold'> Modern JavaScript</span> frameworks. Specializing in
						microservices, cloud solutions, and high-performance architecture.
					</p>

					<div className='flex flex-wrap gap-4 pt-4'>
						<a
							href='mailto:ahmed5_27@hotmail.com'
							className='flex items-center gap-2 px-6 py-3 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-medium transition-all transform hover:-translate-y-1 shadow-lg shadow-blue-500/25'
						>
							<Mail size={20} /> Contact Me
						</a>
						<a
							href='https://www.linkedin.com/in/ahmed527'
							target='_blank'
							rel='noreferrer'
				...
				<div className='flex-1 w-full max-w-lg relative group'>
					<div
						className={`absolute inset-0 bg-gradient-to-r from-blue-500 to-teal-500 rounded-2xl blur-3xl opacity-20 group-hover:opacity-30 transition-opacity duration-500`}
					></div>

					{/* New Profile Card Visual (Not Code) */}
					<div
						className={`relative w-full aspect-square rounded-2xl border ${
							darkMode ? 'bg-slate-800/40 border-slate-700/50' : 'bg-white/60 border-slate-200'
						} backdrop-blur-xl p-8 flex flex-col justify-center gap-6 shadow-2xl`}
					>
						{/* Header / Avatar Area */}
						<div className='flex items-center gap-5 pb-6 border-b border-dashed border-slate-500/30'>
							<div
								className={`w-20 h-20 rounded-2xl flex items-center justify-center text-3xl font-bold bg-gradient-to-br from-blue-500 to-teal-400 text-white shadow-lg`}
							>
								AM
							</div>
							<div>
				...
				<div className='max-w-7xl mx-auto'>
					<div className='text-center mb-16'>
						...
						<p className={`mt-4 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
							Comprehensive stack proficiency from database to frontend.
						</p>
					</div>
						...
						<div key={index} className='relative pl-8 md:pl-0'>
							...
							<div className='flex flex-col md:flex-row gap-8 relative'>
								...
								<div className='md:w-[180px] flex-shrink-0 md:text-right pt-1 md:pr-8'>
									...
									<div className={`text-sm mt-1 ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>{job.location}</div>
									<div
								...
								{/* Timeline Dot */}
								<div
									className={`absolute left-[-37px] md:left-[195px] top-1.5 w-3 h-3 rounded-full border-2 ${
										darkMode ? 'bg-slate-900 border-blue-500' : 'bg-white border-blue-500'
									} z-10`}
								></div>
								...
								>
									<div className='flex flex-col sm:flex-row sm:items-center justify-between mb-2'>
										...
										<span
											className={`text-sm font-semibold px-2 py-0.5 rounded mt-2 sm:mt-0 ${
			...
			<section
				id='projects'
				className={`py-20 px-4 sm:px-6 lg:px-8 ${darkMode ? 'bg-slate-800/30' : 'bg-slate-100/50'}`}
			>
				<div className='max-w-7xl mx-auto'>
					<div className='text-center mb-16'>
						...
						<p className={`mt-4 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
							High-impact solutions delivered for state governments.
						</p>
					</div>
							...
							>
								<div
									className={`h-2 w-full bg-gradient-to-r ${
										index % 2 === 0 ? 'from-blue-500 to-teal-500' : 'from-purple-500 to-pink-500'
									}`}
								></div>
								...
								<div className='flex gap-4'>
									...
									<div>
										...
										<p className={`text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
											Validation of proficiency in Azure development, cloud security, and integration.
										</p>
									</div>
								</div>

								<div className='pt-6 border-t border-slate-700/50'>
									<h4 className='text-sm uppercase tracking-wider font-semibold mb-4 text-slate-500'>
										Currently Pursuing
									</h4>
			...
			{/* Footer */}
			<footer
				className={`py-12 px-4 border-t ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-200'}`}
			>
				<div className='max-w-7xl mx-auto text-center'>
					...
					<div className='flex justify-center gap-8 mb-8'>
						<a
							href='mailto:ahmed5_27@hotmail.com'
							className={`hover:text-blue-400 transition-colors ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}
						>
							<Mail size={24} />
						</a>
						<a
